...
func runTraceCallers(cmd *cobra.Command, args []string) error {
	...
	if traceWorkspace != "" {
		...
		if result.Symbol == nil {
			if traceJSON {
				fmt.Print(captureJSON(result)); return nil
			}
			if traceTOON {
				{ s, e := captureTOON(result); if e != nil { return e }; fmt.Print(s); return nil }
			}
	...
	if len(symbols) == 0 {
		...
		if traceJSON {
			fmt.Print(captureJSON(emptyResult)); return nil
		}
		if traceTOON {
			{ s, e := captureTOON(emptyResult); if e != nil { return e }; fmt.Print(s); return nil }
		}
		fmt.Printf("No symbol found: %s\n", symbolName)
		return nil
	}
